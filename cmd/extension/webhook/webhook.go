...
import (
	...
	admissionvalidator "github.com/gardener/gardener-extension-shoot-pack/pkg/admission/validator"
	...
	cmd := &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Value:       "gardener-extension-shoot-pack-admission",
...
func runWebhookServer(ctx context.Context, cmd *cli.Command) error {
	...
	webhookFuncs := []func(m ctrl.Manager) (*extensionswebhook.Webhook, error){
		func(m ctrl.Manager) (*extensionswebhook.Webhook, error) {
			return admissionvalidator.NewShootValidatorWebhook(m)
		},
	}
